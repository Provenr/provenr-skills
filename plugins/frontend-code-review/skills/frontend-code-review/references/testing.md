...
- Mocking design-system primitives when the real component is practical.
- Mocking base/shared components when the real component is practical.
...
- `clearAllMocks()` in `afterEach` instead of `beforeEach`.

Use real project components for integration behavior. Mock APIs, router, browser shims, or complex providers only when setup would dominate the test.
...
For URL/query-state hooks, flag tests that:
...
Prefer shared testing adapters for the project's URL-state library when available.

## Organization

Flag:

- Component/hook/util tests outside sibling `__tests__/` directories (unless the project convention differs).
- Directory-level reviews that test only the barrel file while other files in scope contain behavior.
