...
- The project's design-system docs and primitive implementations when shared UI components are involved.
- Headless primitive docs and local type contracts when semantics, focus targets, labels, or popup reachability are unclear.
...
- Focus ring applied to the wrong DOM node. Verify the actual focus target, especially with headless controls such as Slider.
...
- Preview/hover content that touch or screen-reader users cannot reach through the trigger's click destination.
...
- Names, labels, file names, or user content lacking `truncate`, `line-clamp`, or `break-words`.
...
The usual layout chain is: container has width constraints, text region uses `min-w-0 flex-1 truncate`, adornments use `shrink-0`.
...
- Fixed widths that break at common mobile/tablet breakpoints.
- Touch targets smaller than ~44×44px for primary actions.
- Horizontal scroll caused by overflow without intentional scroll containers.
