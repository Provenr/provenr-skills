...
Use these rules for component structure, ownership, state, props, effects, and module organization.
...
Accept repeated query calls in siblings when each component independently consumes the data. Cache deduplication alone is not a reason to hoist.

## Component Boundaries

Flag:

- Component files over ~300 lines when the file mixes multiple responsibilities that can be split into focused colocated components, hooks, or utilities.
...
- Refactors of existing navigation, sidebar, dropdown, or list UI that do not preserve behavior-sensitive interactions such as expand/collapse, hover persistence, pin/delete controls, routing, keyboard/focus handling, or open-state ownership.
...
- `React.FC` / `FC`.
- Default exports outside framework-required files (unless the project convention requires them).
...
- Fetch data that belongs in framework APIs or a query library.
...
Use `Link` / `<a>` for normal navigation. Use router APIs for mutation success, guarded redirects, command flows, or form submission side effects.
